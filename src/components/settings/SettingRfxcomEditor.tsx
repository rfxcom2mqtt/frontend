  ...
  return (<>
    {state !== undefined && (<div>
          <FormGroup>
            ...
            <TextField id="rfxcom-port"  variant="outlined" value={state.usbport} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                  setState({...state, usbport: event.target.value});
                }}
                />
            <FormHelperText>Location of the adapter.</FormHelperText>
            <FormLabel>Debug : </FormLabel>
            <Switch {...rfxcomDebuglabel} checked={state.debug} onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
